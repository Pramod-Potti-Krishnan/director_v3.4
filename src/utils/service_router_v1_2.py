...
class ServiceRouterV1_2:
    ...
    def _is_analytics_slide(self, slide: Slide) -> bool:
        ...
        CRITICAL v3.4.2: Analytics Service only accepts L01/L02/L03 layouts.
        Slides with L25 (content) or L29 (hero) layouts MUST be routed to
        Text Service even if they have analytics keywords.

        Args:
            slide: Slide to check

        Returns:
            True if analytics slide with compatible layout, False otherwise
        """
        # Must have analytics classification
        if slide.slide_type_classification != 'analytics':
            return False

        # v3.4.2 Guard: Analytics Service only accepts L01/L02/L03 layouts
        if hasattr(slide, 'layout_id') and slide.layout_id not in ('L01', 'L02', 'L03'):
            logger.warning(
                f"Analytics slide {getattr(slide, 'slide_id', 'unknown')} has incompatible "
                f"layout '{slide.layout_id}'. Analytics requires L01/L02/L03. "
